...
let main args =
    ...
    let config = { App.defaultConfig with FramesPerSecond = fps }
    ...
    let nameFocusId = App.registerWidget "name" (InputWidgets.textInputHandler nameInput) state
    let nameFocused = App.getFocused "name" state
    
    // Text input for email
    let emailInput = InputWidgets.createTextInput "Enter email..." ""
    let _emailFocusId = App.registerWidget "email" (InputWidgets.textInputHandler emailInput) state
    ...
    let _newsletterFocusId = App.registerWidget "newsletter" (InputWidgets.checkboxHandler newsletterCheckbox) state
    ...
    let _termsFocusId = App.registerWidget "terms" (InputWidgets.checkboxHandler termsCheckbox) state
    ...
    let _themeRadioFocusId = App.registerWidget "theme" (InputWidgets.radioGroupHandler themeRadio) state
    ...
    let _countrySelectFocusId = App.registerWidget "country" (InputWidgets.selectHandler countrySelect) state
    let countrySelectFocused = App.getFocused "country" state
    
    // Submit button
    let mutable submitCount = 0
    let submitButton = InputWidgets.createButton "Submit" (fun () ->
        submitCount <- submitCount + 1)
    let _submitFocusId = App.registerWidget "submit" (InputWidgets.buttonHandler submitButton) state
    let submitFocused = App.getFocused "submit" state
    
    // Reset button
    let resetButton = InputWidgets.createButton "Reset" (fun () ->
        nameInput.Value.Set("")
    ...
    let _resetFocusId = App.registerWidget "reset" (InputWidgets.buttonHandler resetButton) state
    let resetFocused = App.getFocused "reset" state
    
    // Quit button
    let quitButton = InputWidgets.createButton "Quit" (fun () ->
        App.stop state)
    let _quitFocusId = App.registerWidget "quit" (InputWidgets.buttonHandler quitButton) state
    ...
    // Subtitle
    let subtitle = Widget.constant "Use Tab to navigate, Enter/Space to interact, Q to quit"
    ...
    let statusContent =
        AVal.map4 (fun name email newsletter terms ->
    ...
    let statusContent2 =
        AVal.map3 (fun theme country ticks ->
            let lines = [
    ...
    let progressValue = AVal.map (fun t -> float (t % 100L) / 100.0) (CVal.value state.Ticks)
    ...
    let mainContent = View.responsiveRow [] (CVal.value state.WindowDimensions.Width) 4 formContent statusWidget
